...
- ✅ **Generic Webhook Trigger Plugin** - For advanced webhook handling
...
2. **Update GitHub Webhook:**
   ```yaml
   GitHub Repository → Settings → Webhooks → Edit webhook
...
#### **Step 11: Configure Branch Filtering**
...
pipeline {
    ...
    stages {
        stage('Branch Check') {
            steps {
                script {
                    echo "Triggered by branch: ${env.ref}"
